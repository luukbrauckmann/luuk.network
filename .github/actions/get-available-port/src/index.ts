...
import { exchangeApiKeyForToken, getAppConfiguration } from "@local/shared";
...
try {
  ...
  const container = appConfig.containerTemplates.find(
      ...
      container.endpoints?.forEach((endpoint) => {
        endpoint.cdn?.portMappings?.forEach((portMapping) => {
          if (
            ...
            portMapping.containerPort > highestPort
          ) {
