...
import type { AllPagesQuery, AllPagesQueryVariables, SiteLocale } from '@generated/datocms';
...
export const loader: Loader = {
  ...
  load: async ({ config, parseData, store }) => {
    ...
    const parsedData = await Promise.all(data.map(async (item) => await parseData({ id: `${item.locale}/${item.path}`, data: item })));
    parsedData.map((parsedItem) => store.set({ id: `${parsedItem.locale}/${parsedItem.path}` , data: parsedItem }));
  },
